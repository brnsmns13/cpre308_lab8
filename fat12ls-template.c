...
void decodeBootSector(struct BootSector * pBootS, unsigned char buffer[])
{
    ...
    char f = buffer[i++];
    char s = buffer[i++];
    //pBootS->iBytesSector = endianSwap((unsigned char)buffer[i++], (unsigned char)buffer[i++]);
    pBootS->iBytesSector = endianSwap(f, s);

    // Read sectors/cluster, Reserved sectors and Number of Fats
    f = buffer[i++];
    s = 0;
    pBootS->iSectorsCluster = endianSwap(f, s);

    f = buffer[i++];
    s = buffer[i++];
    pBootS->iReservedSectors = endianSwap(f, s);

    f = buffer[i++];
    s = 0;
    pBootS->iNumberFATs = endianSwap(f, s);

    // Read root entries, logicical sectors and medium descriptor
    f = buffer[i++];
    s = buffer[i++];
    pBootS->iRootEntries = endianSwap(f, s);

    f = buffer[i++];
    s = buffer[i++];
    pBootS->iLogicalSectors = endianSwap(f, s);

    f = buffer[i++];
    s = 0;
    pBootS->xMediumDescriptor = endianSwap(f, s);

    // Read and covert sectors/fat, sectors/track, and number of heads
    f = buffer[i++];
    s = buffer[i++];
    pBootS->iSectorsFAT = endianSwap(f, s);

    f = buffer[i++];
    s = buffer[i++];
    pBootS->iSectorsTrack = endianSwap(f, s);

    f = buffer[i++];
    s = buffer[i++];
    pBootS->iHeads = endianSwap(f, s);

    // Read hidden sectors
    f = buffer[i++];
    s = buffer[i++];
    pBootS->iHiddenSectors = endianSwap(f, s);
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 32)   {
    	if ( 1 ) {
    		// Display filename
    		printf("%s\t", toDOSName(string, buffer,  i + 0x0)  );
    		// Display Attributes
            key = buffer[i + 0x0b];
    		printf("%s\t", parseAttributes(string, key)  );
    		// Display Time
            f = buffer[i + 0x16];
            s = buffer[i + 0x17];
            utime = endianSwap(f, s);
    		printf("%s\t", parseTime(string, utime )  );
    		// Display Date
            f = buffer[i + 0x18];
            s = buffer[i + 0x19];
            utime = endianSwap(f, s);
    		printf("%s\t", parseDate(string, utime )  );
    		// Display Size
    		printf("%d\n", endianSwap(buffer[i + 0x20], buffer[i + 0x21]) );
    	}
    }
...
char * parseAttributes(char string[], unsigned char key)
{
    ...
    if(!key) { strcat(string, "R"); }
    if(key & 0x10) { strcat(string, "H"); }
    if(key & 0x20) { strcat(string, "S"); }
    if(key & 0x50) { strcat(string, "A"); }
...
char * toDOSName(char string[], unsigned char buffer[], int offset)
{
    ...
    for(i = offset; i < offset + 8; i++)
    {
        ...
        string[i-offset] = temp;
    }

    strcat(string, ".");
    strncat(string, buffer + offset + 8, 3);
    return string;
